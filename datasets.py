...
def make_normal(img, with_condition=True):
    ...
    data = np.clip(
        ((img - np.amin(img)) + 0.00001) / ((np.amax(img) - np.amin(img)) + 0.00001),
        0,
        1,
    )
    if not with_condition:
        ...
        cvar = makeConditionVar(
            data
        )  # conditioning variable which we assume d/t in every dataset
...
def grayIntensity(
    x, equalize=False, treshold=120, b_treshold=0, c_treshold=0, nb_channels=4
):  # 120
    ...
    mask_clean = cv2.morphologyEx(
        mask, cv2.MORPH_OPEN, np.ones((3, 3), int), iterations=1
    )  # cler ver smaller detached objects
    big_mask = np.dstack(
        [mask_clean] * nb_channels
    )  # multi-channel mask based on number of channels specifid
...
def channelIntensity(
    x,
    channel_tresholds=[0.75, 0.65, 0.65, 0.65],
    channel="last",
    approach="max",
    nb_channels=4,
):  # 0.75, 0.65, 0.65]
    ...
    if approach == "mode":
        ...
        mask = aa * bb * cc
    mask_clean = cv2.morphologyEx(
        cc.astype(np.uint8), cv2.MORPH_OPEN, np.ones((3, 3), int), iterations=1
    )  # replace cc with mask
...
class TrainDataset(Dataset):
    def __init__(
        ...
        self.fake_dataset_size = (
            fake_dataset_size if fake_dataset_size is not None else len(self.lbl_dir)
        )
        print("============================================")
        print(
        ...
        if (self.fake_dataset_size is not None) and (
            ...
            print(
                ...
                len(self.img_dir),
                "Number of test images after restriction",
                len(self.lbl_dir),
            )

        self.image_array = self.image2Array(
            self.img_dir, condition=self.with_condition
        )  # read image arra
        self.mask_array = self.computeMask(
            self.lbl_dir,
            func=self.func,
        ...
        )  # read image arra and compute array
        ...
        mod_a = [
            np.where(masked_a[i] == 0, self.image_array[i], masked_a[i])
            for i in range(len(masked_a))
        ]
    ...
    def image2Array(self, images, condition=False):
        return [
            make_normal(imread(image), with_condition=condition) for image in images
        ]
    ...
    def computeMask(
        ...
        if func == "NDVI":
            masks = [
                ndviMasker(img, treshold=ndvi_treshold, nb_channels=nb_channels)
                for img in arrays
            ]
        else:
            if func == "grayIntensity":
                masks = [
                    grayIntensity(
                        img,
                        equalize=equalize,
                        treshold=intensity_treshold,
                        c_treshold=c_treshold,
                        b_treshold=b_treshold,
                        nb_channels=nb_channels,
                    )
                    for img in arrays
                ]
                ...
                masks = [
                    channelIntensity(img, nb_channels=nb_channels) for img in arrays
                ]
...
class TestDataset(Dataset):
    def __init__(
        ...
        self.image_array = self.image2Array(
            self.img_dir, normalize=True, condition=self.with_condition
        )  # read image arra
        self.lbl_array = self.image2Array(
            self.lbl_dir, normalize=False
        )  # read image arra and compute array
        if self.with_mask:
            self.mask_array = self.computeMask(
                self.img_dir,
                ...
                b_treshold=self.b_treshold,
                nb_channels=self.nb_channels,
            )  # self.img_dir is the directory that contained

            assert len(self.image_array) == len(
                self.mask_array
            ), f"image {len(self.image_array)} and label {len(self.mask_array)} numbers are not the same"
    ...
    def image2Array(self, images, normalize=False, condition=False):
        if not normalize:
            ...
            return [
                make_normal(imread(image).astype(float), with_condition=condition)
                for image in images
            ]
    ...
    def computeMask(
        ...
        masks = [
            mask
            if mask is not None
            else np.zeros((256, 256, nb_channels), dtype=np.uint8)
            for mask in masks
        ]
        return masks
