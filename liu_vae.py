...
class VAE_LIU(nn.Module):
    ...
    def __init__(self, img_size, nb_channels, latent_img_size, z_dim,
        ...
        print(f'Model will initiated with liuvae-->: {self.liu_vae}')
    ...
    def get_activation_gradient(self, conv_id):
        #print('The layer IDS --> <--', self.gradients.keys())
    ...
    def get_activations(self, x, conv_id):
        ...
        #print(f'Trying activations for layer id: {conv_id}')
        if conv_id == 'conv_1':
            return x1
        elif conv_id == 'conv_2':
            return x2
    ...
    def loss_function(self, recon_x, x):
        ...
        loss_dict = {
            'loss': loss,
            'rec_term': rec_term,
            'beta*kld': self.beta * kld  # was '-beta*kld'
        }
        if self.disc_module:
