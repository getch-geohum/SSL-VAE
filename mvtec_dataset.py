...
def computemask(size=200, points=10, rad=0.5, edgy=0.5, plot=False):
    xs, ys = [randrange(0, size) for i in range(points)], [
        randrange(0, size) for i in range(points)
    ]
...
def manipulateImage(image, mask, size=200):
    ...
    X_deformed = elasticdeform.deform_random_grid(
        sample, axis=(0, 1), sigma=sigma, points=5, order=1
    )
...
class MvtechTrainDataset(Dataset):
    ...
    def image2Array(self, file):
        ...
        assert (
            normal_mask.shape == changed_mask.shape
        ), f"Mask shapes {normal_mask.shape} and {changed_mask.shape} are not equal"
    ...
    def __getitem__(self, index):
        ...
        if self.with_prob:
            p = np.where(
                mskm[:, :, 0] == 1,
                np.sum(mskm[:, :, 0]) / (512 * 512),
                1 - (np.sum(mskm[:, :, 0]) / (512 * 512)),
            )
            p = np.dstack([p] * 3)
            return (
                self.transform(nrm),
...
class MvtechTestDataset(Dataset):
    def __init__(self, root, texture="carpet", fake_dataset_size=None, validate=False):
        ...
        self.lbl_dir = [
            img.replace("test", "ground_truth").replace(".png", "_mask.png")
