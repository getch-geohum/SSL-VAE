...
def main(args):
    ...
    folds = list(os.listdir(args.root)) # files to leave on out
    print([f'{i}:{folds[i]}' for i in range(len(folds))]) 
    #folds.remove('Minawao_june_2016')
    #folds.remove('Deghale_Apr_2017')
    #folds.remove('Nguyen_march_2017')

    for fold in ['Kule_tirkidi_jun_2018']:
        print(f'Building model for --> {fold}')
        model = UNet(n_channels=3, n_classes=1)
        model.to(device)
        model.train()

        optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)  # change this
        criterion = RMSELoss()
        ...
        for fold_ in list(os.listdir(args.root)): # loading data by folowing leaving one dataset approach
            if fold_ == fold:
        ...
        train_dataset = camp.CampDatasetUnet(imgs=TRAIN[0], lbls=TRAIN[1], sigma=args.sigma)
        ...
        PATH = os.path.join(args.save_path, f'model/{fold}/weights.pt')
        ...
        #        stp_loss+=loss
        #        c+=1
        #mean_loss = stp_loss/c
        #c = 0
        #if mean_loss<vlos:
       #     torch.save(model.state_dict(), PATH)
      #      vlos=mean_loss
      #  else:
      #      pass # also possible to force early stopping
      #  model.train()
        ...
        os.makedirs(args.save_path + f'/report/{fold}', exist_ok=True)
        os.makedirs(args.save_path + f'/preds/{fold}', exist_ok=True)

        test_dataset = camp.CampDatasetUnet(imgs=TEST[0], lbls=TEST[1], sigma=args.sigma)
        test_dataloader = DataLoader(test_dataset, batch_size=args.batch_size, pin_memory=True, shuffle=True)
        ...
        with open(f'{args.save_path}/report/{fold}/rocauc_count.txt', 'a') as txt:
        ...
        for idx in range(10):
            ...
            ax[1].set_title('ref:{0:.2f}'.format(ref.sum()))
            ax[2].set_title('pred:{0:.2f}'.format(pred.sum()))
            for i in range(3):
                ax[i].set_xticks([])
            plt.subplots_adjust(hspace = 0.001, wspace = 0.001)
            plt.savefig(f'{args.save_path}/preds/{fold}/image_{idx}.png', dpi=100, bbox_inches='tight')
            plt.close('all')
...
def parse_args():
    ...
    parser.add_argument("--save_path", type=str, default="/Unet_output")
